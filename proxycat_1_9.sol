...
contract ProxyKitty {
      function read(address target1,address taget2,uint start,uint end) public view returns (string memory) {}
}
...
contract proxycat{ 
    ...
    function cat(string memory label,address target1,address target2,uint start,uint end) public returns(string memory){
        return ProxyKitty(kitties[label].kitty).read(target1,target2,start,end);
    }
    
    function addCall(address kitty,string memory label,address target1,address target2,uint start,uint end) public returns(bool){
        kitties[label]=Kitty(kitty,label,target1,target2,start,end);
        return true;
    }
  ...
  abi.encodePacked(namehash, keccak256(abi.encodePacked(_name)))
   ...
   function DFO(address dfo_address)public view returns(string memory){
    ...
    function graph(address guy) public view returns(string memory) {
       HappyGraph hg=HappyGraph(0x9e117B122c47435FB9ae35271403566fF1005268);
